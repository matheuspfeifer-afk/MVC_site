...
<header class="admin-header">
    ...
    <nav>
        ...
        <a href="/admin/usuarios" class="btn-nav" style="background: #4e73df;">Usuários</a>
    ...
    <?php if (($pagination['totalPages'] ?? 1) > 1): ?>
        <div style="display:flex;justify-content:center;gap:8px;padding:20px;flex-wrap:wrap;">
            ...
            <?php if ($pagination['page'] > 1): ?>
                <a href="?page=<?= $pagination['page'] - 1 ?>&nome=<?= urlencode($filtros['busca'] ?? '') ?>&status=<?= urlencode($filtros['status'] ?? '') ?>&order=<?= urlencode($filtros['ordem'] ?? '') ?>&dir=<?= urlencode($filtros['dir'] ?? '') ?>"
                   class="btn-nav"
            ...
            <?php for ($i = 1; $i <= $pagination['totalPages']; $i++): ?>
                <a href="?page=<?= $i ?>&nome=<?= urlencode($filtros['busca'] ?? '') ?>&status=<?= urlencode($filtros['status'] ?? '') ?>&order=<?= urlencode($filtros['ordem'] ?? '') ?>&dir=<?= urlencode($filtros['dir'] ?? '') ?>"
                   class="btn-nav"
                   style="
                           background: <?= $i === $pagination['page'] ? '#4e73df' : '#e9ecef' ?>;
